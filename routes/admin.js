...
router.get("/cinemaAdd",function(req,res){
...
router.post("/movieAdd",checkAdmin, upload.single("poster"),async function(req,res){
...
router.post("/movieUpdate",checkAdmin,upload.single("poster"),async function(req,res){
    ...
    const update = {
        ...
        syn: req.body.syn,
        updated: Date.now(),
        updateddBy: req.session.admin.id,
    }
    if(req.file){ 
        try{
    ...
    const data = await Movies.findByIdAndUpdate(req.body.id,{
        $set: update});
        console.log(data);
...
router.get("/timeManagement", checkAdmin, async function (req,res){
    const movies = await Movies.find({endDate: {$gte: Date.now()}}).populate("cinemaId");
    console.log(movies);
